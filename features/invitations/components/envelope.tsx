 ...
 * open like a real envelope lid, using a real 3D rotation (rotateX)
 * around its top edge, with perspective set on the parent so it reads as
 * genuinely three-dimensional.
 */
export function Envelope({ isOpening }: EnvelopeProps) {
  return (
    <div
      className="relative w-[70vw] max-w-[280px] aspect-[4/3]"
      style={{ perspective: 1200 }}
    >
      <div className="absolute inset-0 rounded-md bg-card overflow-hidden">
      ...
      <motion.div
        className="absolute left-1/2 top-[46%] w-11 h-11"
        style={{ x: "-50%", y: "-50%" }}
